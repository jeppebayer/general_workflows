...
def migration_simulation_simsfs(config_file: str = glob.glob('*config.y*ml')[0]):
    ...
    if not os.path.isdir(f'{new_wd}/simulated_SFS_3rates/'):
        os.makedirs(f'{new_wd}/simulated_SFS_3rates/')
    ...
    if not os.path.isdir(f'{new_wd_noSingletons}/simulated_SFS_3rates/'):
        os.makedirs(f'{new_wd_noSingletons}/simulated_SFS_3rates/')

    if not os.path.isdir(f'{new_wd_noSingletons}/simulated_SFS_3rates/'):
        os.makedirs(f'{new_wd_noSingletons}/simulated_SFS_3rates/')
    
    for element in range(0, len(DESCRIBER_LIST)):
        if not os.path.isdir(f'{new_wd}/simulated_SFS_3rates/{DESCRIBER_LIST[element]}'):
            os.makedirs(f'{new_wd}/simulated_SFS_3rates/{DESCRIBER_LIST[element]}')
        
        if not os.path.isdir(f'{new_wd_noSingletons}/simulated_SFS_3rates/{DESCRIBER_LIST[element]}'):
            os.makedirs(f'{new_wd_noSingletons}/simulated_SFS_3rates/{DESCRIBER_LIST[element]}')
    ...
    for number in range(0, len(SFS_FILES_LIST)):
        ...
        sfs_file_new = f'{new_wd}/simulated_SFS_3rates/{describer}/{os.path.basename(file)}'
        print(sfs_file_new)

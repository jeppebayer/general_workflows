...
def update_matrix(input_file, matrix_file, string_species, output_file=None, prefix=""):
    ...
    for name, value in zip(sample_names, values):
        # Ensure species column exists first
        if string_species not in df.columns:
            df[string_species] = float('nan')

        # If the row is missing, add it (now safe since we have at least one column)
        if name not in df.index:
            df.loc[name] = pd.Series(dtype=float)

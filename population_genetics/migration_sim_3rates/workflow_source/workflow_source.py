...
def migration_simulation(config_file: str = glob.glob('*config.y*ml')[0]):
    ...
    if not os.path.isdir(f'{new_wd}/2dSFS_3rates'):
        os.makedirs(f'{new_wd}/2dSFS_3rates')
    if not os.path.isdir(f'{new_wd}/2dSFS_3rates/adata_prep'):
        os.makedirs(f'{new_wd}/2dSFS_3rates/adata_prep')
    ...
    if not os.path.isdir(f'{new_wd_noSingletons}/2dSFS_3rates'):
        os.makedirs(f'{new_wd_noSingletons}/2dSFS_3rates')
    ...
    awk_parse_pops_from_vcf(VCF_FILE, f'{new_wd}/2dSFS_3rates/adata_prep/{species_abbreviation(SPECIES_NAME)}_parsed_pops.tsv')
    print(f'Individual IDs and counters saved to {new_wd}/2dSFS_3rates/adata_prep/{species_abbreviation(SPECIES_NAME)}_parsed_pops.tsv')
    # Generate all possible pairs where counter2 >= counter1
    awk_create_pairs(f'{new_wd}/2dSFS_3rates/adata_prep/{species_abbreviation(SPECIES_NAME)}_parsed_pops.tsv', f'{new_wd}/2dSFS_3rates/adata_prep/{species_abbreviation(SPECIES_NAME)}_pop_pairs.tsv')
    print(f'Individual pairs saved to {new_wd}/2dSFS_3rates/adata_prep/{species_abbreviation(SPECIES_NAME)}_pop_pairs.tsv')
    ...
    make_ready_for_2dSFS_pairs = gwf.target_from_template(
            ...
            template=prepare_data(
                vcf_file=VCF_FILE,
                pop_pairs_file=f'{new_wd}/2dSFS_3rates/adata_prep/{species_abbreviation(SPECIES_NAME)}_pop_pairs.tsv',
                output_prefix=species_abbreviation(SPECIES_NAME),
                working_dir=f'{new_wd}/2dSFS_3rates/')
        )
    ...
    input_dict_list = create_input_dict_2dSFS(pair_file = f'{new_wd}/2dSFS_3rates/adata_prep/{species_abbreviation(SPECIES_NAME)}_pop_pairs.tsv', include_list = INCLUDE_POPS, exclude_list = EXCLUDE_POPS)
    ...
    neutral_vcf_files_runtemplate_map = gwf.map(
        ...
        extra = {'out2_file': make_ready_for_2dSFS_pairs.outputs['outfile_dat'], 
                 'working_directory': f'{new_wd}/2dSFS_3rates/'})
    ...
    #neutral_vcf_files_runtemplate_map_noSingletons = gwf.map(
     #   #name=create_run_name_fsc_pair,
      #  template_func = pair_2DSFS_map_target_noSingletons,
       # inputs = input_dict_list,
        #extra = {'out2_file': make_ready_for_2dSFS_pairs.outputs['outfile_dat'], 
         #        'working_directory': f'{new_wd_noSingletons}/2dSFS_3rates/',
          #       'working_directory_alldat': f'{new_wd}/2dSFS_3rates/'})
    ...
    for migration_divide in range(MIN_MIG_DIVIDE, MAX_MIG_DIVIDE, MIGRATION_DIVIDE_INTERVAL):
        ...
        setup_run_FastSimCoal = gwf.map(
            ...
            inputs = input_dict_list_FSC[0:70],
            #inputs = input_dict_list_FSC[0:1],
